...
//		print128_bit(diff_XMM);
	...
	total_difference = SRS_m128i_sse(diff_XMM);
...
int bit_vec_hamming(char* read, char* ref, int length, int max_error) {
	...
	__m128i diff_XMM0 = _mm_xor_si128(read_XMM0, ref_XMM0);
	__m128i diff_XMM1 = _mm_xor_si128(read_XMM1, ref_XMM1);

	__m128i diff_XMM = _mm_or_si128(diff_XMM0, diff_XMM1);
	diff_XMM = _mm_and_si128(diff_XMM, mask);
	
	int total_difference = hamming_m128i_sse(diff_XMM);
