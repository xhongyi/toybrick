...
//		strncpy(ref, init_all_NULL, 128);
		...
		for (i = 0; i < PREFETCH_HEAD_START; i++){
		   ...
		   */
  		  _mm_prefetch(read_strs + random_indices[i], _MM_HINT_T0);
		  _mm_prefetch(read_strs[random_indices[i]].c_str() + CACHE_LINE_SIZE, _MM_HINT_T0);

		  _mm_prefetch(ref_strs + random_indices[i], _MM_HINT_T0);
		  _mm_prefetch(ref_strs[random_indices[i]].c_str() + CACHE_LINE_SIZE, _MM_HINT_T0);
		...
		for (i = 0; i < read_size - PREFETCH_HEAD_START; i++) {
		  ...
		  _mm_prefetch(read_strs + next_prefetch, _MM_HINT_T0);
                  _mm_prefetch(read_strs[next_prefetch].c_str() + CACHE_LINE_SIZE, _MM_HINT_T0);

                  _mm_prefetch(ref_strs + next_prefetch, _MM_HINT_T0);
                  _mm_prefetch(ref_strs[next_prefetch].c_str() + CACHE_LINE_SIZE, _MM_HINT_T0);
